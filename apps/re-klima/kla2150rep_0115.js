...
(function () {
    ...
    $(document).on("change", ".kla2150repsplitsw", function (evt) {
         ...
         * und übergeben
         */
        var saverow = uihelper.cloneObject(klirow);
        var summeryears = {}; // alle Jahre
        var winteryears = {}; // alle Jahre
        /**
         * erst Sommer, dann Winter vorbereiten und abrufen, dann alte saverow restore
         */
        var state = $(this).prop("checked"); // neuer Status der Checkbox
        var canvasid = $(this).parent().find("canvas").attr("id");
        if (typeof canvasid === "undefined" || canvasid === null) {
        ...
        if (canvasid.endsWith("grachart")) {
            // daraus kann ursprüngliche cid berechnet werden
            var oldcid = canvasid.replace("grachart", "");
            async.waterfall([
                    function (cb2100s1) {
                        // summer 1 - Vorbereitung aus saverow
                        // in klirow.data werden je Jahr die Jahreszeiten aufbereitet
                        // Ersetzung echter Werte durch null - Sommer von April bis September
                        var yearkeys = Object.keys(klirow.years);
                        var summeryear = []; // ein Jahr
                        var winteryear = []; // ein Jahr
                        for (var ivar = 0; ivar < yearkeys.length; ivar++) {
                            var actyear = yearkeys[ivar];
                            var iyear = parseInt(actyear);
                            summeryear = [];
                            winteryear = [];
                            var endw1 = 31 + 28 + 31; // Januar - März
                            var ends1 = endw1 + 30 + 31 + 30 + 31 + 31 + 30; // April - September
                            var endw2 = ends1 + 31 + 30 + 31; // Oktober - Dezember
                            if (uihelper.isleapyear(iyear)) {
                                endw1++;
                                ends1++;
                                endw2++;
                            }
                            var lastday = saverow.years[actyear].length;
                            for (var iday = 0; iday < lastday; iday++) {
                                if (iday < endw1) {
                                    summeryear.push(null);
                                    winteryear.push(saverow.years[actyear][iday]);
                                } else if (iday < ends1) {
                                    summeryear.push(saverow.years[actyear][iday]);
                                    winteryear.push(null);
                                } else if (iday < lastday) {
                                    summeryear.push(null);
                                    winteryear.push(saverow.years[actyear][iday]);
                                }
                            }
                            summeryears[actyear] = uihelper.cloneObject(summeryear);
                            winteryears[actyear] = uihelper.cloneObject(winteryear);
                        }

                        // neue cid berechnen und div dazu anlegen
                        var newcid = "D" + Math.floor(Math.random() * 100000) + 1 + "L";
                        $("#" + oldcid).parent().append($("<div/>", {
                            id: newcid,
                            "text-align": "left",
                            float: "left",
                            width: "95%"
                    ...
                    function (ret, cb2100w1) {
                        // winter 1
                        // Ersetzung echter Werte durch null - Sommer Oktober bis Dezember und von Januar bis März
                        // neue cid berechnen und div dazu anlegen
                        var newcid = "D" + Math.floor(Math.random() * 100000) + 1 + "L";
                        $("#" + oldcid).parent().append($("<div/>", {
                            id: newcid,
    ...
    kla2150rep.klitemp2 = function (cid, title, klirow, cb2100k) {
        ...
        try {
            ...
            var graph = window.charts[chartcanvas.substr(1)];
            /**
            ...
            var mingradient = minresult.equation[0].toFixed(3);
            var minyIntercept = minresult.equation[1].toFixed(2);
            ...
            var avgresult = regression.linear(avgregvals, {
                order: 2,
                precision: 3
            });
            var avggradient = avgresult.equation[0].toFixed(3);
            var avgyIntercept = avgresult.equation[1].toFixed(2);
            ...
            var maxresult = regression.linear(maxregvals, {
                order: 2,
                precision: 3
            });

            var maxgradient = maxresult.equation[0].toFixed(3);
            var maxyIntercept = maxresult.equation[1].toFixed(2);

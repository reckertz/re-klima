...
(function () {
     ...
     */
    kli9010srv.ecadcomplete = function (gblInfo, gbldbx, fs, path, rootname, async, stream, StreamZip, readline, sys0000sysx, kli9020funx, req, resx, callback45) {
        /*
        forcedata: forcedata,
        fullname: fullname,
        timeout: 70 * 60 * 1000,
    ...
    kli9010srv.putecaddata = function (statable, soutable, years, fldtable, targetvar, targetelem, async, callback47) {
        ...
        async.eachSeries(souids, function (souid, nextsouid) {
                ...
                var latobj = kli9020fun.getlatfieldsp(starec.latitude);
     ...
     */
    kli9010srv.getallclimatedata = function (gbldb, async, sys0000sys, kli9020fun, uihelper, req, reqparm, res, callbackcd) {
    ...
    var yearlats = {};
    var linereader = null;
    kli9010srv.ghcninv2stations = function (gblInfo, gbldb, fs, async, stream, readline, sys0000sys, kli9020fun, req, res, supercallback1) {
        latprofile = {};
        yearlats = {};
        ...
        async.waterfall([
                ...
                function (ret, callback1) {
                    ...
                    rl.on('line', function (line) {
                        ...
                        var latobj = kli9020fun.getlatfieldsp(lat);
     ...
     */
    kli9010srv.crutem4complete = function (gblInfo, gbldb, fs, path, rootname, async, stream, readline, sys0000sys, kli9020fun, req, res, supercallback) {
                            ...
                            if (line.substr(4, 1) === " ") {
                                ...
                                if (Object.keys(starec).length > 0 && typeof starec.tavg !== "undefined") {
                                    ...
                                    sys0000sys.setonerecord(gbldb, async, null, reqparm, res, function (res, ret1) {
                                        ...
                                        starec = {};
                                        kli9010srv.crutem4staline(ret, line, starec, kli9020fun);
                                        // rl.resume(); // holt den nächsten Satz, auch aus waterfall
                                        // return;
                                        cb(true);
                                        return;
    ...
    kli9010srv.crutem4staline = function (ret, line, starec, kli9020fun) {
        /**
        ...
        */
        ...
        if (starec.latitude === "-999") {
            ...
            latfields = kli9020fun.getlatfieldsp(starec.latitude);
        }
     ...
     * returns function (res, ret)
     */
    kli9010srv.ghcncomplete = function (gblInfo, gbldb, fs, path, rootname, async, stream, readline, sys0000sys, kli9020fun, req, res, supercallback) {
    ...
    kli9010srv.ghcndat2all = function (fullfilename, selyears, variablename, ret1, gblInfo, gbldb, fs, async, stream, readline, sys0000sys, req, res, callbackgc3) {
        var ret = ret1;
        ...
        if (!fs.existsSync(fullfilename)) {
            ret1.message += " Datei nicht vorhanden:" + fullfilename;
            callbackgc3(res, ret1);
            return;
        }
        ...
        */
        var LineByLineReader = require('line-by-line');
        var rl = new LineByLineReader(fullfilename);
        // event is emitted after each line
        rl.on('line', function (line) {
            var that = this;
            rl.pause();
            ...
            if (datrecord.stationid !== vglstationid && vglstationid.length > 0) {
                // alten Satz ausgeben
                var reqparm = {};
                reqparm.selfields = {
                    source: ret.source,
                    stationid: vglstationid
                };
                reqparm.updfields = {};
                reqparm.updfields["$setOnInsert"] = {
                    source: ret.source,
                ...
                reqparm.updfields["$set"][variablename] = {};
                reqparm.updfields["$set"][variablename].years = years;
                var erg = kli9010srv.getyearsels(years);
                reqparm.updfields["$set"].fromyear = erg.fromyear;
                reqparm.updfields["$set"].toyear = erg.toyear;
                reqparm.updfields["$set"].anzyears = erg.anzyears;
                reqparm.updfields["$set"].realyears = Object.keys(years).length;
                reqparm.table = "KLISTATIONS";
                sys0000sys.setonerecord(gbldb, async, null, reqparm, res, function (res, ret) {
                    stationcounter++;
                    if (stationcounter % 100 === 0) {
                        console.log(stationcounter + " Update-Done:" + vglstationid);
                    }
                    years = {};
                    vglstationid = datrecord.stationid;
                    kli9010srv.loadghcnyear(line, lastbis, datrecord, years);
                    rl.resume(); // holt den nächsten Satz, auch aus waterfall
                });
            } else {
                vglstationid = datrecord.stationid;
        ...
        rl.on('end', function (line) {
            ...
            reqparm.table = "KLISTATIONS";
            sys0000sys.setonerecord(gbldb, async, null, reqparm, res, function (res, ret1) {
                console.log(stationcounter + ". Last Update-Done:" + vglstationid);
                console.log('Total lines : ' + counter);
                callbackgc3(res, ret);
                return;
            });
    ...
    /**
     * kli9010srv.getyearsels (years)
     * return erg mit fromyear, toyear, anzyears
     */
    kli9010srv.getyearsels = function (years) {
        var erg = {
            fromyear: null,
            toyear: null,
            anyears: 0
        ...
        for (var year in years) {
            if (years.hasOwnProperty(year)) {
                if (erg.fromyear === null) {
                    erg.fromyear = parseInt(year);
                } else if (parseInt(year) < erg.fromyear) {
                    erg.fromyear = parseInt(year);
                }
                if (erg.toyear === null) {
                    erg.toyear = parseInt(year);
                } else if (parseInt(year) > erg.toyear) {
                    erg.toyear = parseInt(year);
                }
            }
        }
        erg.anzyears = erg.toyear - erg.fromyear + 1;
        erg.fromyear = "" + erg.fromyear;
        erg.toyear = "" + erg.toyear;
        return erg;
    };


    /**
     * datrecord nach years übernehmen
     */
    kli9010srv.loadghcnyear = function (line, lastbis, datrecord, years) {
        ...
        for (var imon = 0; imon < 12; imon++) {
            ...
            for (var ifld = 0; ifld < monschema.length; ifld++) {
                ...
                if (name === "value") {
                    ...
                    if (monvalue === "999" || monvalue === "-999" || monvalue === "-9999") {
                        monvalue = null;
                    }
                    years[year].push(monvalue);
                }
                dis += parseInt(len);
            }
        }
    ...
    /**
     * ghcndat2latitudes - dat für tmin und tmax lesen, 
     * fullname, timeout, pivotopcode, selyears
     * Zugriff KLISTATIONS und Update/Insert KLISTATIONS
     * returns res, ret mit yearlats?
     */

    kli9010srv.ghcndat2latitudes = function (gblInfo, gbldb, fs, path, rootname, async, stream, readline, sys0000sys, kli9020fun, req, res, supercallback2) {
        latprofile = {};
        yearlats = {};
        console.log("ghcndat2latitudes gestartet");
        var fullname = "";
        if (req.query && typeof req.query.fullname !== "undefined" && req.query.fullname.length > 0) {
            fullname = req.query.fullname;
        }
        ...
        async.waterfall([
                ...
                function (res, ret, callback2) {
                    ...
                    sys0000sys.getonerecord(gbldb, async, null, reqparm, res, function (res, ret1) {
                        ...
                        if (ret1.error === false && typeof ret1.record !== "und ined" && ret1.record !== null) {
                    ...
                    sys0000sys.getallrecords(gbldb, async, null, reqparm, res, function (res, ret3) {
                        if (ret3.error === true) {
                            ...
                            if (ret3.records !== "undefined" && ret3.records !== null && ret3.records.length > 0) {
                                stations = {};
                                xstations = {};
                                for (var irec = 0; irec < ret3.records.length; irec++) {
                                    var record = ret3.records[irec];
                                    var lat = record.latitude;
                                    var latn = record.latn;
                                    var lats = record.lats;
                                    stations[record.stationid] = lats;
                                    xstations[record.stationid] = {
                                        latn: latn,
                                        lats: lats,
                                ...
                                callback2("Error", res, {
                                    error: true,
                                    message: "KLISTATIONS nicht gefunden"
                                });
                                return;
                            }
                        }
                ...
                function (res, ret, callback2) {
                    var fullfilename = ret.fullname;
                    var variablename = "";
                    if (fullfilename.indexOf("tavg") >= 0) {
                        variablename = "tavg";
                    } else if (fullfilename.indexOf("tmin") >= 0) {
                        fullfilename.replace(/tmin/i, "tmax");
                        variablename = "tmax";
                    }
                    kli9010srv.ghcndat2obj(fullfilename, selyears, variablename, ret, gblInfo, gbldb, fs, async, stream, readline, sys0000sys, req, res, function (res, ret1) {
                        callback2(null, res, ret1);
                ...
                function (res, ret, callback2) {
                    var fullfilename = ret.fullname;
                    // nicht wenn tavg, sonst zuerst tmax und dann tmin
                    if (fullfilename.indexOf("tavg") >= 0) {
                        callback2(null, res, ret);
                        return;
                    } else if (fullfilename.indexOf("tmax") >= 0) {
                        fullfilename.replace(/tmax/i, "tmin");
                    }
                    var variablename = "tmin";
                    kli9010srv.ghcndat2obj(fullfilename, selyears, variablename, ret, gblInfo, gbldb, fs, async, stream, readline, sys0000sys, req, res, function (res, ret) {
                        callback2(null, res, ret);
                        return;
                    });
                },
                ...
                function (res, ret, callback2) {
                    ...
                    kli9020fun.addTemplist2yearlats(yearlats);
            ...
            function (error, res, ret) {
                console.log("ghcndat2latitudes beendet");
                ret.yearlats = yearlats;
     ...
     */
    kli9010srv.crutem42latitudes = function (gblInfo, gbldb, fs, path, rootname, async, stream, readline, sys0000sys, kli9020fun, uihelper, req, res, supercallback2) {
        latprofile = {};
        yearlats = {};
        ...
        async.waterfall([
                ...
                function (res, ret, callback2) {
                    ...
                    kli9010srv.crutem42obj(fullfilename, selyears, variablename, ret, gblInfo, gbldb, fs, async, stream, readline, sys0000sys, kli9020fun, uihelper, req, res, function (res, ret1) {
                        callback2(null, res, ret1);
                        return;
                    });
                },
    ...
    /**
     * crutem42obj lesen eine dat-Datei mit Auswertung fullfilename und variablename auf tavg
     * returns res, ret
     */
    kli9010srv.crutem42obj = function (fullfilename, selyears, variablename, ret1, gblInfo, gbldb, fs, async, stream, readline, sys0000sys, kli9020fun, uihelper, req, res, callback3) {
        var ret = ret1;
        var sels = selyears.split(",");
        var counter = 0;
        var html = "";
        ...
        rl.on('line', function (line) {
            ...
            if (line.substr(4, 1) === " ") {
                ...
                */
                ...
                starec.lats = latfields.lats;
                /**
                 * KLISTATIONS fortschreiben CRUTEM4
                 */
                var reqparm = {};
                reqparm.selfields = {
                    source: "CRUTEM4",
                ...
                var invrecord = uihelper.cloneObject(starec);
                delete invrecord.source;
                delete invrecord.stationid;
                delete invrecord.history;
                reqparm.updfields["$set"] = invrecord;
            ...
            // starec und datrec jetzt zu Ausgabe aufbereiten,
            // starec wird nur gepuffert, verarbeitet wird nur datrec, wenn Treffer
            /**
             * Selektion year, kommasepariert, * und - zulässig
             */
            if (typeof datrecord.year === "undefined" || datrecord.year === null) {
                that.resume(); // holt den nächsten Satz, auch aus waterfall
                return;
            }
            if (!kli9010srv.hasyear(datrecord.year, selyears)) {
                that.resume(); // holt den nächsten Satz, auch aus waterfall
                return;
            }
             ...
             *              months - Monatstabelle mit tmin, tmax
             */
            var stationid = starec.stationid;
            var year = datrecord.year;
            var lats = starec.lats;
            // year, lats und stationid sind hier bekannt
            if (typeof yearlats[year] === "undefined") {
                yearlats[year] = {};
            }
            ...
            if (yearlats[year][lats].stations.indexOf(stationid) < 0) {
                yearlats[year][lats].stations += stationid + ";";
                yearlats[year][lats].xstations.push({
                    id: starec.stationid,
                    latn: starec.latn,
                    lon: starec.longitude,
                    lat: starec.latitude,
                    name: starec.name
                });
            }
            ...
            var mon;
            for (var imo1 = 0; imo1 < datrecord.months.length; imo1++) {
                value = datrecord.months[imo1].tavg;
                mon = "M" + ("00" + (imo1 + 1)).slice(-2);
                if (typeof yearlats[year][lats].months[mon] === "undefined") {
                    yearlats[year][lats].months[mon] = {
                        tmin: null,
    ...
    /**
     * vglipccfiles - file1, file2, selyear, selmonth, sellat
     * return vgldata als Array von Sätzen
     */
    kli9010srv.vglipccfiles = function (gblInfo, gbldb, fs, path, rootname, async, stream, readline, sys0000sys, req, res, supercallback7) {
        var file1 = "";
        if (req.query && typeof req.query.file1 !== "undefined" && req.query.file1.length > 0) {
            file1 = req.query.file1;
        }
        var file2 = "";
        if (req.query && typeof req.query.file2 !== "undefined" && req.query.file2.length > 0) {
            file2 = req.query.file2;
        }
        var sellat = "";
        if (req.query && typeof req.query.sellat !== "undefined" && req.query.sellat.length > 0) {
            sellat = req.query.sellat;
        }
        var selyear = "";
        if (req.query && typeof req.query.selyear !== "undefined" && req.query.selyear.length > 0) {
            selyear = req.query.selyear;
        }
        var selmonth = "";
        if (req.query && typeof req.query.selmonth !== "undefined" && req.query.selmonth.length > 0) {
            selmonth = req.query.selmonth;
        }
        var vgldata = [];
        async.waterfall([
                function (callback7) {
                    ...
                    rl.on('line', function (line) {
                        ...
                        if (line.substr(4, 1) === " ") {
                            ...
                            // 
                            var latfields = kli9020fun.getlatfields(starec.latitude);
                            starec.lat = latfields.lat;
                            starec.latn = latfields.latn;
                            starec.lats = latfields.lats;
                            // Dezimalstelle einfügen
                            starec.longitude = starec.longitude.substr(0, starec.longitude.length - 1) + "." + starec.longitude.substr(starec.longitude.length - 1);
                            starec.latitude = starec.latitude.substr(0, starec.latitude.length - 1) + "." + starec.latitude.substr(starec.latitude.length - 1);
                            that.resume(); // holt den nächsten Satz, auch aus waterfall
                            return;
                        }
                        // starec und datrec jetzt zu Ausgabe aufbereiten,
                         ...
                         * Selektion year, kommasepariert, * und - zulässig
                         */
                        if (datrecord.year !== selyear) {
                            that.resume(); // holt den nächsten Satz, auch aus waterfall
                            return;
                        }
                        if (starec.lat !== sellat && starec.latn !== sellat && starec.lats !== sellat) {
                            that.resume(); // holt den nächsten Satz, auch aus waterfall
                            return;
                        }
                        // jetzt den Monat holen und die Werte dazu
                        var stationid = starec.stationid;
                        var year = datrecord.year;
                        var lats = starec.lats;
                        // imo1 aus selmonth
                        if (selmonth.length > 0) {
                            var imo1 = parseInt(selmonth) - 1;
                            var selvalue = datrecord.months[imo1].tavg;
                            vgldata.push({
                                source: "CRUTEM4",
                                longitude: starec.longitude,
                                latitude: starec.latitude,
                                selyear: datrecord.year,
                                selmonth: selmonth,
                                sellat: sellat,
                                selstationid: starec.stationid,
                                selname: starec.name,
                                tavg: selvalue
                            });
                        } else {
                            ...
                            for (var imon = 0; imon < 12; imon++) {
                                vglrec.selmonth = "M" + ("00" + (imon + 1)).slice(-2);
                                vglrec.tavg = datrecord.months[imon].tavg;
                                vgldata.push(vglrec);
                            }
                        }
                        that.resume(); // holt den nächsten Satz, auch aus waterfall
                    ...
                    rl.on('close', function (line) {
                        console.log('Total lines : ' + counter);
                        callback7(null, res, ret);
                    });
                },
                function (res, ret, callback7) {
                    var seltype1 = "";
                    if (file1.indexOf("v3") >= 0) {
                        seltype1 = "GHCN";
                    } else {
                        seltype1 = "GHCN4";
                    }
                    ret.seltype1 = seltype1;
                        ...
                        if (ret3.error === true) {
                            ...
                            if (ret3.records !== "undefined" && ret3.records !== null && ret3.records.length > 0) {
                                ...
                                for (var irec = 0; irec < ret3.records.length; irec++) {
                                    ...
                                    var latparms = kli9020fun.getlatfieldsp(record.latitude);

                                    stations[record.stationid] = latn;
                                    xstations[record.stationid] = {
                ...
                function (res, ret, callback7) {
                    ...
                    rl.on('line', function (line) {
                        ...
                        var stationid = datrecord.stationid;
                        var lat = stations[stationid];
                        var starec = xstations[stationid];
                        var lats = ""; // lat.substr(0, 1) + ("000" + lat.substr(1)).slice(-3);
                        ...
                        if (selmonth.length > 0) {
                            ...
                            var imo1 = parseInt(selmonth) - 1;
                            if (datrecord.variable === "TMIN") {
                                vglrec.tmin = datrecord.months[imo1];
                            } else if (datrecord.variable === "TMAX") {
                                vglrec.tmax = datrecord.months[imo1];
                            } else {
                                vglrec.tavg = datrecord.months[imo1];
                            }
                            vgldata.push(vglrec);
                        } else {
                            var vglrec = {
                            ...
                            for (var imon = 0; imon < 12; imon++) {
                                vglrec.selmonth = "M" + ("00" + (imon + 1)).slice(-2);
                                if (datrecord.variable === "TMIN") {
                                    vglrec.tmin = datrecord.months[imon];
                                } else if (datrecord.variable === "TMAX") {
                                    vglrec.tmax = datrecord.months[imon];
                                } else {
                                    vglrec.tavg = datrecord.months[imon];
                                }
                                vgldata.push(vglrec);
                            }
                        }
     ...
     * function (res, ret)
     */
    kli9010srv.getstationdata = function (gblInfo, gbldb, fs, async, stream, readline, crg, sys0000sys, kli9020fun, req, res, callbacktd) {

...
(function () {
    ...
    /**
     * convertETRS89UTM - Umrechnen in longitude und latitude
     * @param {*} utmew - "e32"
     * @param {*} utmx - "2935xx" - Zone ist 32! e = east
     * @param {*} utmns - "n32"
     * @param {*} utmy - "56452xx" - Zone ist 32, n = north
     * aus xx wird 50
     * returns "ret-Objekt" mit error, message, longitude, latitude
     */
    uihelper.convertETRS89UTM = function (utmew, utmx, utmns, utmy) {
        var latlon = new Array(2);
        var zone, southhemi;
        var x, y;

        if (isNaN(parseFloat(utmx))) {
            return {
                error: true,
                message: "Please enter a valid easting in the x field."
            };
        }

        x = parseFloat(utmx);

        if (isNaN(parseFloat(utmy))) {
            return {
                error: true,
                message: "Please enter a valid northing in the y field."
            };
        }
        ...
        if (isNaN(parseInt(zone))) {
            return {
                error: true,
                message: "Please enter a valid UTM zone in the zone field."
            };
        }
        ...
        if ((zone < 1) || (60 < zone)) {
            return {
                error: true,
                message: "The UTM zone you entered is out of range.  " +
                    "Please enter a number in the range [1, 60]."
            };
        }

        if (utmns.substr(0, 1).toUpperCase() === "S") {
            southhemi = true;
        } else {
            southhemi = false;
        }

        uihelper.UTMXYToLatLon(x, y, zone, southhemi, latlon);
        ...
        return {
            ...
            longitude: uihelper.RadToDeg(latlon[1]),
            latitude: uihelper.RadToDeg(latlon[0]),
        };
    };
     ...
     *
     */
    uihelper.UTMXYToLatLon = function (x, y, zone, southhemi, latlon) {
        var cmeridian;
        var UTMScaleFactor = 0.9996;
        x -= 500000.0;
        x /= UTMScaleFactor;
        ...
        if (southhemi)
            y -= 10000000.0;
        y /= UTMScaleFactor;
        cmeridian = uihelper.UTMCentralMeridian(zone);
        uihelper.MapXYToLatLon(x, y, cmeridian, latlon);
        return;
    };
     ...
     *
     */
    uihelper.UTMCentralMeridian = function (zone) {
        var cmeridian;
        cmeridian = uihelper.DegToRad(-183.0 + (zone * 6.0));
        return cmeridian;
    };
     ...
     *
     */
    uihelper.DegToRad = function (deg) {
     ...
     *
     */
    uihelper.RadToDeg = function (rad) {
     ...
     *
     */
    uihelper.MapXYToLatLon = function (x, y, lambda0, philambda) {
        var phif, Nf, Nfpow, nuf2, ep2, tf, tf2, tf4, cf;
        var x1frac, x2frac, x3frac, x4frac, x5frac, x6frac, x7frac, x8frac;
        var x2poly, x3poly, x4poly, x5poly, x6poly, x7poly, x8poly;

        /* Get the value of phif, the footpoint latitude. */
        phif = uihelper.FootpointLatitude(y);
     ...
     *
     */
    uihelper.FootpointLatitude = function (y) {
